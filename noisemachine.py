            ...
            try:
                ...
                if self.last_button_pressed == moinitored_button:
                    if self.button_event.is_set():
                        ...
                        self.__play_sound(self.buttons[moinitored_button].filename_generators.get(PressType.DOUBLE))

                    else:
                        self.logger.debug('Button {} was single-pressed.'.format(moinitored_button))
                        self.__play_sound(self.buttons[moinitored_button].filename_generators.get(PressType.SINGLE))
                ...
                if split_filename[1] == 'single':
                    if len(split_filename) == 4 and split_filename[2] == 'random':
                        if new_button.filename_generators.get(PressType.SINGLE) is None:
                            new_button.filename_generators[PressType.SINGLE] = FilenameGenerator([(order_number, file)], GeneratorType.RANDOM)

                        else:
                            new_button.filename_generators[PressType.SINGLE].add_filename(file, order_number)

                    elif len(split_filename) == 4 and split_filename[2] == 'sequence':
                        if new_button.filename_generators.get(PressType.SINGLE) is None:
                            new_button.filename_generators[PressType.SINGLE] = FilenameGenerator([(order_number, file)], GeneratorType.SEQUENCE)

                        else:
                            new_button.filename_generators[PressType.SINGLE].add_filename(file, order_number)

                    else:
                        new_button.filename_generators[PressType.SINGLE] = FilenameGenerator([(1, file)], GeneratorType.SINGLE)

                elif split_filename[1] == 'double':
                    if len(split_filename) == 4 and split_filename[2] == 'random':
                        if new_button.filename_generators.get(PressType.DOUBLE) is None:
                            new_button.filename_generators[PressType.DOUBLE] = FilenameGenerator([(order_number, file)], GeneratorType.RANDOM)

                        else:
                            new_button.filename_generators[PressType.DOUBLE].add_filename(file, order_number)

                    elif len(split_filename) == 4 and split_filename[2] == 'sequence':
                        if new_button.filename_generators.get(PressType.DOUBLE) is None:
                            new_button.filename_generators[PressType.DOUBLE] = FilenameGenerator([(order_number, file)], GeneratorType.SEQUENCE)

                        else:
                            new_button.filename_generators[PressType.DOUBLE].add_filename(file, order_number)

                    else:
                        new_button.filename_generators[PressType.DOUBLE] = FilenameGenerator([(1, file)], GeneratorType.SINGLE)
